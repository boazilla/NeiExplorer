...
public class StatePillar {
   // shared resources
   private static StatePillar _instance = new StatePillar();
   ...
   public static StatePillar foundInstance(Context main_context, GoogleApiClient api_Clt) {
   ...
   public int hashStInt(){
      String  _center = getCenterNow(),
              _type   = getTypeWriter(),
              _word   = getWordOfBlake(),
              _rad    = getLookupRadius(),
              _mode   = String.valueOf(_by_keys)+String.valueOf(_by_word);
      String form_hash_target
              = _center +_type +_word +_rad +_mode;
      return form_hash_target.hashCode();
   }
   public String hashStString(){
      return String.valueOf(hashStInt());
   }
   ...
   public void updateFields() {
      ...
      if (shP.contains(key)) {
         _keyword = shP.getString(key, "atm");
      } else {
...
//         https://developer.android.com/training/location/change-location-settings.html
   ...
   public String getCenterNow() {
      return String.format("%1$2.5f,%2$2.4f",_now.latitude,_now.longitude);
      // 2.5,2.4 is ok
   }
