...
//           yell_TYPE          = "yell_type",
   ...
   static final String
           ...
           st_OK              = "OK";
   ArrayList<JustPlace> all_places;
   ...
   protected void onHandleIntent(Intent intent) {
      if (intent != null) {
         ...
         {
            // implicit call of FullSearch() returned TRUE;
            pushPreference( String.valueOf(all_places.size()));
            reflection.DAO.storeSynth(all_places);
         }
      }
   }
   ...
   boolean doFull20Search(String location_, String radius_, String word_, String type_) {
      ...
      String
              ...
              _FULL_SEARCH_T      = "nearbysearch/json?location=%2$s&radius=%3$s&type=%4$s&key=%1$s",
              _FULL_SEARCH_D      = "nearbysearch/json?rankby=distance&location=%2$s&radius=%3$s&keyword=%4$s&type=%5$s&key=%1$s",
              _FULL_SEARCH_DN     = "nearbysearch/json?location=%2$s&radius=%3$s&type=place_of_interest&key=%1$s",
      ...
      int loop = 0, passes_left =3, continued = 3;
ask_url:
      do {
         ...
         try {
            ...
            switch (status) {
               ...
               case st_INVALID_REQUEST:
                  pushPreference("delayed "+ (4-continued) + " red: " + all_places.size());
            ...
            if (bootObj.has("next_page_token") && !bootObj.getString("next_page_token").isEmpty()) {
               String old_token = next_token;
               next_token = bootObj.getString("next_page_token");
               if (old_token.equals(next_token)) {
                  passes_left = 0;
                  next_token = "no_pages";
               }
            }
         ...
         while (loop < 20 && !place_cluster.isNull(loop)) {
            try {
               JSONObject next = place_cluster.getJSONObject(loop);
               all_places.add(new JustPlace(next));
            } catch (JSONException e) {
               e.printStackTrace();
               passes_left = 0;
            }
