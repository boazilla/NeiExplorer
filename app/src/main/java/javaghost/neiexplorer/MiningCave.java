...
/*   CREATE TABLE table1 (
   ...
   public long lastContext(){
      long maxid = 0;
      Cursor cursor = _db.query(TBL_CONTEXTS, new String[] {"MAX("+cl_cID+")"}, null, null, null, null, null);
      if (cursor != null) {
         cursor.moveToFirst();
...
//   public void idx_placeContxt(String focal_, String type_, String word_, String radius_ ){}
//   public void idx_placeContxt(LatLng focal_, String type_, String word_, double radius_ ){}
   ...
   public long storeSynth(ArrayList<JustPlace> setof_places) {
      ...
      if (lastxt == contxt) {
         ...
         cv = new ContentValues();
         try {
            for (int idx = 0; idx < setof_places.size(); idx++) {
               JustPlace idx_place = setof_places.get(idx);
               cv.put(cl_vID, contxt);
               cv.put(cl_IDps, upstore(idx_place));
               cv.put(cl_FAVORED, false);
               Location.distanceBetween(reflection._now.latitude, reflection._now.longitude, idx_place._location.latitude, idx_place._location.longitude, distances);
